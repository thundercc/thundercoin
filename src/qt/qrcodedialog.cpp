...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non THD unit input in lnReqAmount, we generate the URI with THD as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::THD, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
